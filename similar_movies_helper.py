...
def show_similar_movies(movie_id):
    ...
    search_button = tk.Button(year_range_window, text="Search", command=lambda: fetch_similar_movies(movie_id, start_year_entry.get(), end_year_entry.get()))
    search_button.pack()

def fetch_similar_movies(movie_id, start_year, end_year):
    movie = Movie()
    try:
        ...
        def display_similar_movies(page):
            for idx, m in enumerate(filtered_movies[(page - 1) * items_per_page:page * items_per_page]):
                if m.poster_path:
                    ...
                    if response.status_code == 200:
                        ...
                        label = tk.Label(similar_movies_window, text=f"{m.title} ({m.release_date[:4]}) - {m.overview[:100]}")
                        label.image = photo  # Keeping a reference to the image
                        label.grid(row=idx, column=0)
                        image_label = tk.Label(similar_movies_window, image=photo)
                        image_label.image = photo  # Keeping a reference to the image
                        image_label.grid(row=idx, column=1)
                else:
                    tk.Label(similar_movies_window, text=f"{m.title} ({m.release_date[:4]}) - {m.overview[:100]}").grid(row=idx, column=0)

            if len(filtered_movies) > page * items_per_page:
                tk.Button(similar_movies_window, text="Next", command=lambda: display_similar_movies(page + 1)).grid(row=items_per_page, column=1)

        display_similar_movies(page)
